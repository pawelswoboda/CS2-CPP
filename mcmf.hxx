...
namespace CS2_CPP {
   ...
   template<typename COST_TYPE = long long int, typename CAPACITY_TYPE = long int>
      class MCMF_CS2
      {
         ...
         public:
            ...
            // do zrobienia: remove _node_min and _node_max
            long _node_min; // minimal no of nodes
            long _node_max; // maximal no of nodes
            // those are temporary variables only
         ...
         public:
            ...
            ~MCMF_CS2() {
               //static_assert(std::is_integral_type<CAPACITY_TYPE>::value, "CAPACITY_TYPE must be integral");
            }
            ...
            long no_arcs(const long i) const { return _nodes[i+1].first() - _nodes[i].first(); }
            ...
            void set_cap(const long arc_id, const long cap) { 
               assert(cap >= 0);
               _cap[arc_id] = cap; 
            }
            ...
            NODE* stackq_pop() {
               return remove_from_excess_q();
            }
            // utils for buckets;
            // do zrobienia: make BUCKET methods
            ...
            NODE* get_from_bucket( BUCKET *b) {
               assert(nonempty_bucket(b));
               NODE* i = b->p_first();
            ...
            // misc utils;
            bool OPEN(ARC* a) const { return a->rez_capacity() > 0; }
            bool CLOSED(ARC* a) const { return a->rez_capacity() <= 0; }
            price_t REDUCED_COST( NODE* i, NODE* j, ARC* a ) const { return i->price() + a->cost() - j->price(); }
            bool FEASIBLE( NODE* i, NODE* j, ARC* a ) const { return i->price() + a->cost() < j->price(); }
            bool ADMISSIBLE( NODE* i, NODE* j, ARC* a ) const { return OPEN( a ) && FEASIBLE( i, j, a ); }
            bool SUSPENDED( NODE* i, ARC* a ) const { return a < i->first(); }
   ...
   // flags for shortest path computation
...
//#define SUSPENDED( i, a ) ( a < i->first() )
   ...
   template<typename COST_TYPE, typename CAPACITY_TYPE>
      ...
      {
         if ( id < 0 || id > _n ) {
            throw std::runtime_error("Error:  node index out of bound\n");
         }
      ...
      int MCMF_CS2_STAT<COST_TYPE,CAPACITY_TYPE>::check_cs()
      {
         ...
         for (auto* i = this->_nodes.get(); i != this->_sentinel_node; i ++) {
            ...
            for ( ; a != a_stop; a ++) {
               ...
               if ( this->OPEN(a) && (this->REDUCED_COST(i, a->head(), a) < 0) ) {
                  return ( 0);
               }
            }
         }
   ...
   template<typename COST_TYPE, typename CAPACITY_TYPE>
      int MCMF_CS2_STAT<COST_TYPE,CAPACITY_TYPE>::check_eps_opt()
      {
         for (auto* i = this->_nodes.get(); i != this->_sentinel_node; i ++) {
            ...
            auto* a_stop = (i+1)->suspended();
            for ( ; a != a_stop; a ++) {
               if ( this->OPEN(a) && (this->REDUCED_COST(i, a->head(), a) < - this->_epsilon) ) {
                  return ( 0);
               }
            }
         }
